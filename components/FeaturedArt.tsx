...
export default function FeaturedArt() {
  ...
  useEffect(() => {
    // Get latest uploaded art (remove limit if you want ALL)
    const q = query(
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 mt-16 max-w-7xl mx-auto">
      {images.map((img) => (
        <div
          key={img.id}
          className="rounded-xl overflow-hidden border border-white/10 hover:border-brand transition group"
        >
          <img
            src={img.imageUrl}
          ...
          <div className="p-3 text-sm">
            <div className="font-semibold opacity-90">{img.title || "Untitled"}</div>
            <div className="opacity-50 line-clamp-2">{img.prompt}</div>
          </div>
        </div>
      ))}
    </div>
